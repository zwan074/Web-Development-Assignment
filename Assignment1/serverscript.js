...
function hasliked(likes,user,IPaddress) // Submit button clicked
  {

	for (let i = 0; i < IPaddress.length; i++) {
    	if (IPaddress[i].childNodes[0].nodeValue == user ) return true;
    }
...
function fetch_data_from_server_to_display_friendStatus() {
	...
	xhttp.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	var user = xmlDoc.getElementsByTagName("data")[0].getAttribute("userID");
	    	console.log(user);
	    	display_all_friendStatus(this,user);
...
function display_all_friendStatus(xml,user) {
    ...
    for (let i = 0; i < ipInfriendlist.length; i++) {
    	...
    	display_friendStatus(user,friend);
...
function display_friendStatus(user,friend) {
	...
	friendxhttp.onreadystatechange = function() {
	    if (this.readyState == 4 && this.status == 200) {
	    	...
	    	txt = "<table style=\"width:20%; border: 1px solid black; border-collapse: collapse;\">";
    	    txt += "<tr><th>";
	    	txt += "<img src=\"..\/" + friend + "\/" + friend + "profile.PNG\" alt=\"" + friend + " profile picture\" width=\"200\" height=\"20\" \/>" + "<br>";
	    	txt += friend ;
	    	txt += "</th></tr>";
	    	status = xmlDoc.getElementsByTagName("status");
	    	...
	    	for (let i = 0; i < status.length; i++) {
	        	if (status[i].childNodes[0] != null){
	    	        ...
	    	        if (likes[0] != null){
	    	        	IPaddress = likes[0].getElementsByTagName("IPaddress"); 
	    	        	if (hasliked(likes,user,IPaddress) ){
